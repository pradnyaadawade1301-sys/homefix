...
type Review struct {
	...
	CreatedAt    time.Time `json:"created_at"`
	// CustomerName is only populated by ReviewRepository.ListByTechnician
	// (the technician-facing "My Reviews" list) — it's who left the review,
	// so the technician can see who's reviewing them, not just a rating and
	// a comment. Left blank/omitted everywhere else Review is used.
