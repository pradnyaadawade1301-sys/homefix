...
type Review struct {
	...
	CreatedAt    time.Time `json:"created_at"`
	// CustomerName is only populated by read queries that join against
	// users (see ReviewRepository.ListByTechnician) — Create/
	// CreateForConsultation leave it empty since a review's own insert has
	// no reason to look the name up.
