...
func (s *ConsultationService) Escalate(ctx context.Context, consultationID, addressID, problemDescription, notes string, scheduledAt *time.Time) (*models.Booking, error) {
	...
	// No pre-visit fee: the customer pays the full bill (which includes the
	// flat visit charge as a line item) only after the technician has visited
	// and completed the job — see RazorpayService.CreateOrder.
